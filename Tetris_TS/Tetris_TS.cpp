...
Tetris_TS::Tetris_TS() : Parent(WIN_WIDTH, WIN_HEIGHT)
{
	...
	for (int y = 0; y < WIN_HEIGHT; y++)
		for (int x = 0; x < WIN_WIDTH - 13; x++)
			if (x == 0 || x == 16 || y == 21) // 20*15 boarders
				SetChar(x, y, '#');
			else
				SetChar(x, y, '.');
	WriteToBuffer();
...
bool Tetris_TS::CollisionTest(int nx, int ny, int rot) {
	...
	for (int y = 0; y < 4; y++) {
		...
		{
			if (rot>=0)
				 si = Rotate(x, y, rot); //index of the piece
			else 
			...
			if (nx + x >= 0 && nx + x < WIN_WIDTH - 13)
			{
				if (ny + y >= 0 && ny + y < WIN_HEIGHT)
				{
					...
					if ((mShapes[mCurrentPiece][si] == 'X') && (buffer[ny + y][nx + x] != '.')) {
						...
						if ((mShapes[mCurrentPiece][si] == 'X') && (buffer[mCurY + y + 1][mCurX + x] == '.'))
						{
							return true; // other piece boarder
						}
						mCollision = true;
